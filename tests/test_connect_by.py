...
class ConnectByTest(unittest.TestCase):
    def test_to_sql_lowers_basic_connect_by_query(self):
    ...
    def test_to_sql_lowers_connect_by_with_level_filter_and_nocycle(self):
        tree = ibis.table([("id", "int64"), ("parent_id", "int64")], name="tree")
    ...
    def test_to_sql_lowers_connect_by_start_with_in_subquery(self):
        tree = ibis.table([("id", "int64"), ("parent_id", "int64")], name="tree")
    ...
    def test_to_sql_lowers_connect_by_start_with_exists_subquery(self):
        tree = ibis.table([("id", "int64"), ("parent_id", "int64")], name="tree")
        roots = ibis.table([("id", "int64")], name="roots")
    ...
    def test_to_sql_lowers_connect_by_on_derived_input(self):
        tree = ibis.table(
            [("id", "int64"), ("parent_id", "int64"), ("active", "boolean")],
            name="tree",
        )
    ...
    def test_to_sqlglot_lowers_connect_protocol_to_connect_ast(self):
        tree = ibis.table([("id", "int64"), ("parent_id", "int64")], name="tree")
        ...
        self.assertIsInstance(compiled, sge.Select)
        self.assertNotIn("__connect", compiled.sql(dialect="postgres"))
        self.assertIsInstance(compiled.args.get("connect"), sge.Connect)
        self.assertEqual(
            compiled.args["connect"].sql(dialect="postgres"),
            "START WITH t0.id = 1 CONNECT BY PRIOR t0.id = t0.parent_id",
        )
        self.assertEqual(compiled.args["where"].sql(dialect="postgres"), "WHERE LEVEL > 1")

    def test_connect_by_rejects_non_boolean_start_with(self):
        tree = ibis.table([("id", "int64"), ("parent_id", "int64")], name="tree")

        with self.assertRaisesRegex(
            com.IbisTypeError,
            "start_with must be a boolean expression",
        ):
            connect_by(
                tree,
                start_with=tree.id,
    ...
    def test_connect_by_rejects_foreign_key_expressions(self):
        tree = ibis.table([("id", "int64"), ("parent_id", "int64")], name="tree")
        other = ibis.table([("id", "int64")], name="other")

        with self.assertRaisesRegex(
            com.IbisInputError,
            "parent_key must reference only the input table",
        ):
            connect_by(
                tree,
                start_with=tree.id == 1,
    ...
    def test_connect_by_rejects_level_name_conflict(self):
        tree = ibis.table(
        ...
        with self.assertRaisesRegex(
            com.IbisInputError,
            "level_name conflicts with an existing column",
        ):
            connect_by(
                tree,
                start_with=tree.id == 1,
    ...
    def test_to_sql_rejects_scalar_subquery_in_connect_start_with(self):
        tree = ibis.table([("id", "int64"), ("parent_id", "int64")], name="tree")
    ...
    def test_compile_optimize_keeps_lowered_connect_by_query_stable(self):
        tree = ibis.table([("id", "int64"), ("parent_id", "int64")], name="tree")
        expr = connect_by(
            tree,
            start_with=tree.id == 1,
        ...
        self.assertEqual(
            compiled.sql(dialect=DSQLDialect),
    ...
    def test_to_sql_lowers_connect_by_before_joining_to_another_table(self):
        tree = ibis.table([("id", "int64"), ("parent_id", "int64")], name="tree")
        ...
        self.assertNotIn("__connect", sql)
        self.assertEqual(
            sql,
    ...
    def test_to_sql_lowers_connect_by_before_outer_aggregate(self):
        tree = ibis.table([("id", "int64"), ("parent_id", "int64")], name="tree")
    ...
    def test_to_sql_lowers_connect_by_when_used_in_outer_in_subquery(self):
        tree = ibis.table([("id", "int64"), ("parent_id", "int64")], name="tree")
