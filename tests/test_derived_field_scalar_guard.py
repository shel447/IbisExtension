...
class DerivedFieldScalarGuardTest(unittest.TestCase):
    def test_guard_allows_plain_filter_equality(self):
        users = ibis.table([("name", "string")], name="users")

        sql = to_sql(users.filter(users.name == "abc"))

        self.assertEqual(sql, "SELECT t0.name FROM users AS t0 WHERE t0.name = 'abc'")

    def test_guard_allows_plain_filter_isnull(self):
        users = ibis.table([("name", "string")], name="users")

        sql = to_sql(users.filter(users.name.isnull()))

        self.assertEqual(sql, "SELECT t0.name FROM users AS t0 WHERE t0.name IS NULL")

    def test_guard_allows_filter_with_in_subquery(self):
    ...
    def test_guard_allows_filter_with_in_subquery_over_aliased_derived_chain(self):
        table_a = ibis.table([("x", "string"), ("sid", "int64")], name="TableA")
    ...
    def test_guard_allows_connect_by_outer_join(self):
    ...
    def test_guard_rejects_same_table_derived_field_scalar_leak(self):
        table = ibis.table(
    ...
    def test_guard_rejects_cross_table_derived_field_scalar_leak(self):
        table_a = ibis.table(
            [("x", "string"), ("id", "int64"), ("sid", "int64")],
            name="TableA",
        )
    ...
    def test_guard_rejects_flipped_comparison_with_derived_field(self):
        table_a = ibis.table(
            [("x", "string"), ("id", "int64"), ("sid", "int64")],
            name="TableA",
        )
    ...
    def test_guard_rejects_derived_field_inside_arithmetic_expression(self):
        table_a = ibis.table(
            [("x", "string"), ("id", "int64"), ("sid", "int64")],
            name="TableA",
        )
    ...
    def test_guard_rejects_derived_field_inside_function_call(self):
        table_a = ibis.table(
            [("x", "string"), ("id", "int64"), ("sid", "int64")],
            name="TableA",
        )
    ...
    def test_guard_rejects_projected_alias_derived_field_scalar_leak(self):
        table_a = ibis.table([("x", "string"), ("sid", "int64")], name="TableA")
        table_b = ibis.table([("id", "int64")], name="TableB")
        devs = table_a.filter(table_a.x == "abc")
        mid = devs.select(fid=devs.sid)
    ...
    def test_guard_rejects_mutated_alias_derived_field_scalar_leak(self):
        table_a = ibis.table([("x", "string"), ("sid", "int64")], name="TableA")
        table_b = ibis.table([("id", "int64")], name="TableB")
        devs = table_a.filter(table_a.x == "abc")
    ...
    def test_guard_rejects_chained_project_alias_derived_field_scalar_leak(self):
        table_a = ibis.table([("x", "string"), ("sid", "int64")], name="TableA")
        table_b = ibis.table([("id", "int64")], name="TableB")
        devs = table_a.filter(table_a.x == "abc")
    ...
    def test_guard_rejects_view_wrapped_project_alias_derived_field_scalar_leak(self):
        table_a = ibis.table([("x", "string"), ("sid", "int64")], name="TableA")
        table_b = ibis.table([("id", "int64")], name="TableB")
    ...
    def test_guard_unary_predicate_from_projected_alias_derived_field_is_rejected_upstream(
        ...
        with self.assertRaisesRegex(com.IntegrityError, "belong to another relation"):
            table_b.filter(mid.fid.isnull())

    def test_guard_connect_by_start_with_foreign_relation_is_rejected_upstream(self):
        tree = ibis.table([("id", "int64"), ("parent_id", "int64")], name="tree")
        roots = tree.filter(tree.id > 1)

        with self.assertRaisesRegex(
            com.IbisInputError,
            "start_with must reference only the input table",
        ):
            connect_by(
                tree,
